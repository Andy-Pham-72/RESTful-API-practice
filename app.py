...
@app.cli.command("dc_create")
def db_create():
    db.create_all()
    print("Database Created!")

@app.cli.command("dc_drop")
def db_drop():
    db.drop_all()
    print("Database dropped!")

@app.cli.command("dc_seed")
def db_seed():
    ...
    test_user = User(first_name="Andy",
                     last_name="Pham",
                     email="test@test.com",
                     password="P@ssw0rd"
                     )
...
# same logic with above "/parameters" but different url structure
...
@app.route('/action')
def action():
    """example: http://127.0.0.1:5000/action?x=3&y=5
            => { "result" : 15 }
            """
...
def update_planet():
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(message="You updated planet"), 202
    else:
        return jsonify(message="That planet doesn't exist"), 404
